...
const ProfileConnect: React.FC<ProfileConnectProps> = ({
  ...
  return (
      <div className="relative"> {/* this div will wrap the button and the menu */}
        <button
        ...
        {active && showMenu && ( // this will render the menu only if active and showMenu are true
            <div className="absolute right-0 mt-2 bg-white border border-gray-300 rounded shadow-lg transition-all duration-300 ease-in-out transform scale-95"> {/* this div will style the menu and add a transition animation */}
              <ul className="py-1"> {/* this ul will contain the menu items */}
                <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer" onClick={activating}> {/* this li will be the disconnect button */}
                  Disconnect
                </li>
              </ul>
            </div>
